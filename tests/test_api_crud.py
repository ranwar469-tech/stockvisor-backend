...
def test_discussions_thread_post_crud_flow(client, auth_headers):
    ...
    if post_id is not None:
        delete_post_response = client.delete(f"/discussions/posts/{post_id}", headers=auth_headers)
        assert delete_post_response.status_code == 204
        post_id = None

    if thread_id is not None:
        delete_thread_response = client.delete(f"/discussions/threads/{thread_id}", headers=auth_headers)
        assert delete_thread_response.status_code == 204
        thread_id = None
